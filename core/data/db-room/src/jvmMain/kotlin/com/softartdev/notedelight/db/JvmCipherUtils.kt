...
object JvmCipherUtils {
    ...
    private const val JDBC_ENCRYPTED_PREFIX = "jdbc:sqlite:file:"
    ...
    private fun buildEncryptedUrl(dbPath: String, password: String): String {
        ...
        return "$JDBC_ENCRYPTED_PREFIX$dbPath?cipher=$SQLCIPHER_CIPHER&legacy=$SQLCIPHER_LEGACY&key=$encodedPassword"
    }

    @Suppress("SameParameterValue")
    private fun getConnection(url: String, isEncrypted: Boolean = false): Connection = when {
        isEncrypted -> try {
            ...
            driver.connect(url, props)
                ?: throw SQLException("Failed to connect to encrypted database")
        } catch (e: Exception) {
    ...
    fun decrypt(password: String, dbName: String) {
        ...
        try {
            ...
            val versionResult: ResultSet =
                connection.createStatement().executeQuery("PRAGMA user_version;")
            ...
            val attachStatement =
                connection.prepareStatement("ATTACH DATABASE ? AS plaintext KEY ''")
            ...
            try {
                ...
                for ((tableName, createSql) in tableSchemas) {
                    val quotedTableName = when {
                        createSql.contains("`$tableName`") -> "`$tableName`"
                        createSql.contains("\"$tableName\"") -> "\"$tableName\""
                        else -> tableName
                    }
                    val plaintextCreateSql = createSql.replaceFirst(
                        quotedTableName,
                        "plaintext.$quotedTableName",
                    )
                    ...
                    connection.createStatement()
                        .execute("INSERT INTO plaintext.$tableName SELECT * FROM $tableName")
            ...
            val unencryptedUrl = JDBC_PREFIX + newFile.absolutePath
    ...
    fun encrypt(password: String?, dbName: String) {
        ...
        try {
            ...
            val unencryptedUrl = JDBC_PREFIX + originalFile.absolutePath
            ...
            try {
                ...
                if (hasResultSet) {
                    val cipherVersion: ResultSet? = stmt.resultSet
                    if (cipherVersion?.next() == true) {
                        val cipherVer = cipherVersion.getString(1)
                        logger.d { "Encryption is enabled, cipher version: $cipherVer" }
                    }
                    cipherVersion?.close()
                }
                stmt.close()
            ...
            DriverManager.getConnection(unencryptedUrl).use { unencryptedConn: Connection ->
                ...
                for ((tableName, createSql) in tableSchemas) {
                    ...
                    while (dataResult.next()) {
                        ...
                        columns.forEachIndexed { index, colName ->
                            ...
                            when {
                                colType.contains("INTEGER") -> insertStmt.setLong(
                                    index + 1,
                                    dataResult.getLong(colName)
                                )
                                colType.contains("REAL") -> insertStmt.setDouble(
                                    index + 1,
                                    dataResult.getDouble(colName)
                                )
                                colType.contains("BLOB") -> insertStmt.setBytes(
                                    index + 1,
                                    dataResult.getBytes(colName)
                                )
                                else -> {
                                    ...
                                    if (value != null) {
                                        ...
                                        insertStmt.setNull(index + 1, Types.NULL)
                                    }
                                }
                            }
                        }
            ...
            getConnection(encryptedUrl, isEncrypted = true).use { verifyConnection ->
                val verifyResult =
                    verifyConnection.createStatement().executeQuery("PRAGMA user_version;")
